...
class CascadedPhaseGenerator:
    def __init__(
        self, D, dx, dt, batch_size=100, n_cascades=3, double_precision=False
    ) -> None:
        ...
        N_min += (
            1 - N_min % 2
        )  # Make sure that the minimal size in the cascade is odd so it has a central piston pixel
        self.N = int(
            N_min * factor
        )  # [pixels] Number of grid points; Make sure size is subdividable by 3, because each cascade zooms 3X
        ...
        self.init_noise = xp.array(
            rng.normal(size=(self.N, self.N, 1)), dtype=self.datafloat
        )
        # Spatially-dependant phase retardation of the PSD's realisation phase, used to simulate boiling
        self.random_retardation = (
            xp.asarray(
                np.random.uniform(0, 1, size=(self.N, self.N, 1)), dtype=self.datafloat
            )
            * dt
        )
        ...
        self.boiler = (
            lambda f: f * 2 * dx
        )  # just radial linear gradient map with 1.0 at the N/2 distance from the center for temporal evolution
    ...
    def AddLayer(
        self, weight, r0, L0, wind_speed, wind_direction, boiling_factor, PSDs=None
    ):
        self.layers.append(
            Layer(
                self,
    ...
    def GeneratePSDGrids(self):
        ...
        select_middle = lambda N: np.s_[
            N // 2 - N // 6 : N // 2 + N // 6 + N % 2,
            N // 2 - N // 6 : N // 2 + N // 6 + N % 2,
            :,
        ]  # selects the middle 1/3 quandrant of the image
        self.crops = [
            np.s_[0 : self.N, 0 : self.N, :]
        ]  # 1st crop contains the whole image

        for i in range(self.n_cascades):
            self.crops.append(select_middle(self.N // 3**i))
            dx_ = (
                self.dx * 3**i
            )  # every next cascade zooms out frequencies by 3 to capture more low frequencies
            self.fx[..., i], self.fy[..., i], self.f[..., i], self.df[..., i] = (
                self.freq_array(self.N, dx_)
            )  # [1/m]
        _ = self.crops.pop(-1)
    ...
    def GeneratePSDCascade(self, r0, L0):
        ...
        # Pre-allocate arrays
        arrays_shape = [self.N, self.N, self.n_cascades]
        PSD_temporal = np.zeros(
            arrays_shape, dtype=self.datafloat_cpu
        )  # Describes the temporal evolution of the PSD
        PSDs = np.zeros(
            arrays_shape, dtype=self.datafloat_cpu
        )  # Contains spatial frequencies

        for i in range(self.n_cascades):
            PSDs[..., i] = (
                self.vonKarmanPSD(self.f[..., i], r0, L0) * self.df[i] ** 2 * rad2nm**2
            )  # PSD spatial [nm^2/m^2]
        ...
        PSDs[
            ..., -1
        ] *= mask_outer  # The last cascade has the most information about the low frequencies
    ...
    def vonKarmanPSD(self, k, r0, L0):
        ...
        cte = (24 * spc.gamma(6 / 5) / 5) ** (5 / 6) * (
            spc.gamma(11 / 6) ** 2.0 / (2.0 * np.pi ** (11 / 3))
        )
        PSD = r0 ** (-5 / 3) * cte * (k**2 + 1 / L0**2) ** (-11 / 6)
        PSD[PSD.shape[0] // 2, PSD.shape[1] // 2] = (
            0  # Avoid division by zero at the origin
        )
        return PSD
    ...
    def screens_from_PSD_and_phase(self, PSD, PSD_phase):
        ...
        # Perform batched FFT
        if GPU_flag:
            plan = gfft.get_fft_plan(
                PSD_realizations, axes=(0, 1), value_type="C2C"
            )  # for batched, C2C, 2D transform
            phase_batch = cp.real(
                gfft.ifft2(
                    gfft.ifftshift(PSD_realizations, axes=(0, 1)),
                    axes=(0, 1),
                    plan=plan,
                )
                * PSD.size
            ...
            phase_batch = np.real(
                np.fft.ifft2(
                    np.fft.ifftshift(PSD_realizations, axes=(0, 1)), axes=(0, 1)
                )
                * PSD.size
            )
    ...
    def GenerateScreensBatch(
        self, PSDs, PSD_temporal, wind_speed, wind_direction, boiling_factor
    ):
        N = self.N
        dt = self.dt
        ...
        screen_id = (
            xp.arange(self.num_screens, dtype=self.datafloat)
            + self.current_batch_id * self.num_screens
        )
        ...
        tip = xp.array(
            center_grid(xx)[..., None], dtype=self.datafloat
        )  # 1/N-th of it shifts phase screen by 1 pixel
        ...
        for i in range(n_cascades):
            ...
            V = xp.array(
                wind_speed / self.dx * dt, dtype=self.datafloat
            )  # [pixels/step]
            Vx_ = (
                V
                * xp.cos(xp.deg2rad(xp.array(wind_direction, dtype=self.datafloat)))
                / 3**i
            )
            Vy_ = (
                V
                * xp.sin(xp.deg2rad(xp.array(wind_direction, dtype=self.datafloat)))
                / 3**i
            )
            ...
            evolution = (
                tip * Vx_
                + tilt * Vy_
                + self.random_retardation * boiling_factor * PSD_temporal_
            )
            random_phase = xp.exp(
                2j * xp.pi * (screen_id * evolution + self.init_noise)
            )
            ...
            screens_batch[..., i] = self.interpolator.zoom_FFT(
                buffalo[self.crops[i]], i
            )
            # screens_batch[...,i] = self.interpolator.zoom_interp( buffalo[self.crops[i]], i )

        screens_batch[..., -1] -= screens_batch[..., -1].mean(
            axis=(0, 1), keepdims=True
        )  # remove piston again, just in case
        raw_batch[..., -1] -= raw_batch[..., -1].mean(
            axis=(0, 1), keepdims=True
        )  # remove piston again, just in case
        ...
        screens_batch = screens_batch.sum(
            axis=-1
        )  # sum all cascades to W x H x N_screens
