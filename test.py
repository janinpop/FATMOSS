...
hdul = fits.open(
    "/Users/pjanin/Documents/GitHub/FATMOSS/devs/HarmoniLTAO_JQ2_undersampled.fits"
)
...
print(
    f"Time per screen: {np.round(total_time/screen_generator.num_screens).astype(xp.uint16)} [ms]"
)
...
_, _, f_over, df_over = screen_generator.freq_array(
    screen_generator.N * oversampling_factor, dx
)

rad2nm = 500.0 / 2.0 / np.pi  # [nm/rad]

PSD_ultimate = (
    screen_generator.vonKarmanPSD(f_over, r0, L0)
    * df_over**2
    * rad2nm**2
...
def radialize_PSD(PSD, grid, label="", plot=True, fill=False, **kwargs):
    PSD_profile = radial_profile(PSD, (PSD.shape[0] // 2, PSD.shape[1] // 2))[
        : PSD.shape[1] // 2
    ]
